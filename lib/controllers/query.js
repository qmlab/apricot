...
module.exports.findById = function(req, res, next) {
  req.collection.findById(req.params.id, function(e, result){
    if (e) {
      res.status(500).send("Failed to find item by id:" + e)
    }
    else {
      res.send(result)
    }
...
module.exports.getCollections = function(req, res, next) {
  req.db.collectionNames(function(e, items) {
    if (e) {
      res.status(500).send("Failed to find item by id:" + e)
    }
    else {
...
module.exports.mapcount = function(req, res, next) {
  var map = function() {
    emit(this[group], 1)
  }
...
module.exports.count = function(req, res, next) {
  var count = req.collection.count(req)
  res.send(count)
  req.db.close()
  next()
}
