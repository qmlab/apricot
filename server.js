...
var version = nconf.get('app:version')app.use('/', routerAdmin())app.use('' + (baseurl ? baseurl : ''), router())if (version) {  // Versioned URLs  app.use(util.trimTailingSlash('' + (baseurl ? baseurl : '')) + '/' + version, router())  // Older versions  //app.use(util.trimTailingSlash('' + (baseurl ? baseurl : '')) + '/v0.5' + version, router())  //app.use(util.trimTailingSlash('' + (baseurl ? baseurl : '')) + '/v0.2' + version, router())  //...}// Start servervar protocol = nconf.get('server:protocol')var port = nconf.get('server:port')if (protocol === 'http') {  app.listen(port)}else if (protocol === 'https') {  var privateKey  = fs.readFileSync('certs/priv.pem', 'utf8')  var certificate = fs.readFileSync('certs/pub.cer', 'utf8')  var credentials = {key: privateKey, cert: certificate}  var httpsServer = https.createServer(credentials, app)  httpsServer.listen(port)}